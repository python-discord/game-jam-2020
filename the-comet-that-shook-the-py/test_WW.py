import unittest
from backend import (
    _get_count_files_starting_with,
    _get_puzzle_clues,
    _get_fact_template,
)
...
#        template_one_file = _get_fact_template(
#            ["f1_templates_001.temp", "f1_templates_002.temp"]
#        )
...
#            ],
#            [
#                "{1} is to the right of the {3} programmer",
#                "{2} is to the right of the one who uses {6}",
#                "The {4} programmer does not use {6}",
#                "{0} does not use {8}",
#                "The person who uses {8} doesn't program in {4} ",
#                "{2} doesn't program in {4}",
#            ],
#        ]

    def testTemplateNoFile(self):
        self.assertRaises(FileNotFoundError, _get_fact_template, "NotFoundFile.temp")
...
class TestGetPuzzleClues(unittest.TestCase):
    ...
    def testLessSolutions(self):
        self.assertRaises(
            IndexError, _get_puzzle_clues, ["Microsoft"], ["{0} programmers use {1}"]
        )

#    def testMoreSolutions(self):
#        assert _get_puzzle_clues(
#            ["Microsoft", "IBM", "C#", "python"], ["{0} programmers use {2}"]
#        ) == ["Microsoft programmers use C#"]
#
#    def testEqualSolutions(self):
#        assert _get_puzzle_clues(
#            ["Microsoft", "IBM", "C#", "python"],
