...
class MenuView(arcade.View):
    ...
    def on_show(self):
        ...
        toAdd = Deco(arcade.load_texture("dumbInterfaceImages/dirt slope.png"), 1, 1870, 95)
        self.decorations.append(toAdd)

        for i in range(240):
            for y in range(4):
                toAdd = Deco(arcade.load_texture("dumbInterfaceImages/waveDirt.png"), 1, i * 16 - 50,
                             87 - 28 * y)
                self.decorations.append(toAdd)

        e = 0
        for i in range(30):
            toAdd = Deco(arcade.load_texture("dumbInterfaceImages/grass.png"), 1, e * 64 - 50, 100)
            self.decorations.append(toAdd)
            e += 1

        toAdd = Deco(arcade.load_texture("dumbInterfaceImages/grass slope.png"), 1, 1870, 110)
        self.decorations.append(toAdd)

        for i in range(30):
            toAdd = Deco(arcade.load_texture("dumbInterfaceImages/grass.png"), 1, e * 64 + 30, 100)
            self.decorations.append(toAdd)
            e += 1

        for i in range(5):  # TODO: clouds are glitchy when they're near the edge
            toAdd = Clouds(arcade.load_texture("dumbInterfaceImages/cloud.png"), 0.3, random.randint(-232, 1032),
    ...
    def set_buttons(self):
        self.theme.add_button_textures("dumbInterfaceImages/startButton.png", None,
                                       "dumbInterfaceImages/startButton.png", None)
        self.button_list.append(
            Start(self, SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 50, 320, 320, theme=self.theme))

        self.theme.add_button_textures("dumbInterfaceImages/backButton.png", None,
                                       "dumbInterfaceImages/backButton.png", None)
        self.button_list.append(BackButton(self, 2050, 500, 32, 32, theme=self.theme))

        self.theme.add_button_textures("dumbInterfaceImages/levelButton1.png",
                                       "dumbInterfaceImages/levelButton1.png",
                                       "dumbInterfaceImages/levelButton2.png")
        levelNum = 1
        for y in range(2):
            for x in range(5):
...
class GameOverView(arcade.View):
    def on_show(self):
        ...
        self.gameOverText.textures = [arcade.load_texture('dumbInterfaceImages/gameOverText.png')]
    ...
    def set_buttons(self):
        self.theme.add_button_textures("dumbInterfaceImages/restart.png", None,
                                       "dumbInterfaceImages/restart.png", None)
...
class Backdrop(arcade.Sprite):
    def __init__(self, x):
        super().__init__()
        self.texture = arcade.load_texture("dumbInterfaceImages/menuBackground.png")
        self.scale = 1
